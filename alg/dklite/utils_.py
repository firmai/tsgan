...
def remove_uncertain_data(var_0_tr, var_1_tr, T_true_train, Y_hat_train):
    ...
    var_train = np.swapaxes(var_1_tr + var_0_tr, 0, 1)

    residual = np.square(T_true_train.reshape((-1, 1)) - (Y_hat_train[:, 1] - Y_hat_train[:, 0]).reshape((-1, 1)))

    mean_rmse1, mean_rmse2, indices, indices2, sorted_var = two_error(np.squeeze(residual), np.squeeze(var_train))
